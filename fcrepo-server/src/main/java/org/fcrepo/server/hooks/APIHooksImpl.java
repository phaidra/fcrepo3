...
/* Hugh Barnard changed to Logger and LoggerFactory.getLogger everywhere January 2015 */
	protected static Logger log = LoggerFactory.getLogger(APISOAPHooksImpl.class);	
	 ...
	 * @throws APIHookException If the remote call went wrong
	 */
	public abstract String runHook(String method, DOWriter w, Context context, String pid, Object[] params) throws APIHookException;
	 ...
	 * @throws APIHookException If parsing or processing failed
	 */
	protected String processResults(String hookResultXML, DOWriter w, Context context) throws APIHookException
	{	
		...
		{
			throw new APIHookException("Error parsing hook XML: "+ex.getMessage(), ex);
		}
		...
		if(nl==null)
		{
			throw new APIHookException("Error processing hook XML: no 'verdict' node found");
		}
		Element el = (Element)nl.item(0);
		if(el==null)
		{
			throw new APIHookException("Error processing hook XML: no 'verdict' node found (2)");
		}
		...
		throws APIHookException
	{
		try 
		{
			...
			org.fcrepo.server.storage.types.AuditRecord audit=new org.fcrepo.server.storage.types.AuditRecord();
			...
			if(orig==null)
			{
				log.debug("replaceIntoDatastream: creating new datastream ("+datastream+")");
			...
			{
				log.debug("replaceIntoDatastream: updating existing datastream ("+datastream+")");
		...
		{
			throw new APIHookException("replaceIntoDatastream failed: "+e.getMessage(), e);
		}
	}
		...
		throws APIHookException
	{
		...
		{
			throw new APIHookException("purgeDatastream failed: "+e.getMessage(), e);
		}
	}
